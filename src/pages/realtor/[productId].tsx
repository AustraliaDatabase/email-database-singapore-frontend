...
const RealtorDatabaseByState = (props: ICompanyDatabaseByState) => {
    ...
    <>
      ...
      <PublicLayout>
        <NextSeo
          ...
          canonical={`${process.env.NEXT_PUBLIC_BASE_URL}/realtor/${currentObject.url
            ?.split("/")
            .join("")}`}
        />
        <MainProductMainView
...
export default RealtorDatabaseByState;
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  if (params?.productId?.toLowerCase() !== "list-of-all-email-lists") {
    const productResponse = instance.post(
      // @ts-ignore
      `/realtorProductOld/${params?.productId?.toLowerCase()}`
    );
